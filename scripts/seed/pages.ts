...
<p><strong>7. Disclaimer:</strong> We are Web Based Community Supported Website and We Mostly Collect data from Open Source Resources, Search Engines and Free Content Providers. All Published &amp; User Uploaded Content supposed to be Collected from Open Source Resources.</p>
<h2>Change</h2>
...
export async function seedPages() {
  ...
  for (const page of PAGES) {
    const existing = await db.select({ id: pages.id }).from(pages).where(eq(pages.slug, page.slug)).limit(1);
